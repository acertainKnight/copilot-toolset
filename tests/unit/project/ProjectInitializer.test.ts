...
describe('ProjectInitializer', () => {
  ...
  describe('Project Structure Analysis', () => {
    ...
    it('should respect maximum depth parameter', async () => {
      ...
      const result = await projectInitializer.initialize(deepProjectDir, 2);

      // Should stop at depth 2, so level3 and level4 shouldn't be in the output
  ...
  describe('COPILOT.md Generation', () => {
    it('should create main COPILOT.md file', async () => {
      ...
      const content = await fs.readFile(copilotMdPath, 'utf-8');
      expect(content).toContain('Project Context for GitHub Copilot');
    ...
    it('should create COPILOT.local.md if it doesn\'t exist', async () => {
      const projectDir = await createMockProject(tempDir, 'nodejs');

      await projectInitializer.initialize(projectDir);

      const localMdPath = path.join(projectDir, 'COPILOT.local.md');
      expect(await fs.access(localMdPath).then(() => true, () => false)).toBe(true);

      const content = await fs.readFile(localMdPath, 'utf-8');
      expect(content).toContain('Local Project Overrides');
      expect(content).toContain('Local Development Notes');
    });
    ...
    it('should include project metadata in COPILOT.md', async () => {
      ...
      expect(content).toContain('Generated automatically by Copilot MCP Toolset');
    });
  });
  ...
  describe('Memory Bank Initialization', () => {
    ...
    it('should create project configuration file', async () => {
      ...
      expect(await fs.access(configPath).then(() => true, () => false)).toBe(true);
      ...
      expect(config).toHaveProperty('memoryEnabled');
  ...
  describe('Error Handling', () => {
    it('should handle non-existent project directory', async () => {
      ...
      await expect(projectInitializer.initialize(nonExistentDir))
        .rejects.toThrow();
    });
    ...
    it('should handle very large project structures', async () => {
      ...
      const result = await projectInitializer.initialize(largeProjectDir, 2);
  ...
  describe('Template System', () => {
    it('should have templates for different project types', async () => {
      ...
      expect(nodeResult).toContain('nodejs');
